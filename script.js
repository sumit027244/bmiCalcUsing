...
function getUserInput(){
    ...
    if(activeForm === "bmi-si"){
        ...
        heightin = document.getElementById('in').value,
        weightKg = document.getElementById('kg').value;
        
        status = checkInputStatus([age, heightin, weightKg]);

        if(status === true){
            return calculateBMI({
                gender,
                age,
                height: parseFloat(heightin)*0.0254,
...
function printBMIResult(BMIInfo){
    document.getElementById('bmi-value').innerHTML = `${BMIInfo.BMI} kg/inch<sup>2</sup>`;
